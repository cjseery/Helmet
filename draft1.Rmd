...
```
In order to see how stiffnessFrac and dampingFrac change over time for all foams, we plotted a scatter plot for each vs cycle. Dots of the same color were measured from the same foam sample. In the first plot, StiffnessFrac seems to spread out in an umbrella shape, showing that as cycle increases the sample values not only change, but get further apart. Cycle number looks like it has a relationship with stiffnessFrac, but that relationship looks different for each foam sample.
...
These plots separate the log(stiffnessFrac) vs log(cycle) graph from above to show a better representation on how the interaction between chemical index and porosity impact stiffnessFrac. The top row shows only high porosity and the bottom row shows only low porosity samples. From left to right, the columns show chemical index 79, 100 and 121. We can see that the chemical index 79 has many samples where log(stiffnessFrac) trends upwards, whereas chemical indices 100 and 121 mostly trend downwards. This indicates a relationship between log(stiffnessFrac) and chemical index. Downward trends mean the stiffness decreases as cycle increases, which is not what we are looking for in a durable foam.

We also see an effect from porosity. For chemical index 79, low porosity foam trends downward more often than high porosity foam, and for chemical index 100, low porosity foam trends upward more often than high porosity foam. So not only does porosity affect log(stiffnessFrac), but it effects it different ways for different porosities. This suggests a porosity:chemIndex interraction term would improve our model.
