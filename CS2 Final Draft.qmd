...
We reformatted the data from wide to long to ensure each observation represented one measurement, not one foam sample. Measurements taken beyond 10,000 cycles were removed to maintain consistency across observations, and because Moira identified this as the most relevant time frame.
...
The histogram in @fig-1 shows the distribution of compression frequencies among the foam samples revealing that certain compression frequencies were tested more frequently than others. These non-uniformities in sample distribution might impact the reliability of conclusions drawn from the analysis.
...
To select our variables, we started with a mixed model where all variables aside from log(cycle) were treated as level two variables and log(cycle) was treated as a level one variable. The level two variables on their own will impact the intercept of our model and the interaction between log(cycle) and the other variables will impact the slope of our model. After starting with this full model, we manually removed variables from the model to increase the significance of the coefficients and to minimize the REML. Once a base model was created, we then used ANOVA testing to determine if the different levels of each predictor were significant and to compare against the full model. Based on this testing, we were able to create a model using the following predictors:
...
For the purpose of this paper, these are the variables we will be using in our model. We did find that there is a significant relationship between stiffness fraction and an interaction between amplitude and frequency. Unfortunately, the addition of this term lowered the significance of the relationship between the response and chemical index. Since chemical index is one of the focuses of this experiment, we decided to continue with our original model instead of including this term. 
...
All samples have a stiffness fraction of 1 at cycle zero. The structure of a mixed model creates individual intercepts for each sample so for this model, we will be using samples starting at the first cycle measurement after cycle zero and going to cycle 10000. The alpha terms in the model will explain the impact of the covariates at the initial time recorded and the beta terms will explain the impact of the covariates on the stiffness fraction as time goes on.

From @Table-1 we see a high significance in our coefficients. Although not every variable is significant on it's own, the interaction with time is. For example, chemical index does not appear to have a significant relationship with the response on it's own, but one level of chemical index has a significant relationship when interacted with cycle. The interaction terms in this model are an indicator of how the response will change based on both a covariate and time. From this summary, amplitude appears to have the largest positive impact on initial stiffness fraction and the largest negative impact as time progresses.

```{r, echo = FALSE}
#| echo: false
#| label: Table-1
#| warning: false
#| fig-cap: "Coefficients for Stiffness Fraction Model"
#| fig-align: "center"
#| fig-width: 7
#| fig-height: 4
library(car)
...
Although @Table-1 indicates there is significance to the different levels of chemical index, the output below in @Table-2 confirms that there is a significant difference between at least on of the levels of chemical index. To further assess the significane of the model, we explored the residual plots and found that this model meets the linearity, normality, and equal variance conditions. These plots can be further explored in the appendix.

```{r, echo = FALSE}
#| echo: false
#| label: Table-2
#| warning: false
#| fig-cap: "Anova Analysis of Stiffness Fraction Model"
#| fig-align: "center"
#| fig-width: 7
#| fig-height: 4
Anova(FactorModel1)
...
Lastly, given that we are using a mixed effects model, it is necessary to explore the random effects associated with the model. From @fig-6, we notice that as the intercept increases, the slope decreases. This is a very defined trend and supports our conclusion that a mixed model should be used. We would expect to see that each of the foams starts at a similar stiffness fraction and as time increases, there would be deviations based on the other predictors. This plot confirms this behavior.
...
The summary of this model shows a high significance in almost all of our coefficients through small p-values (less than 0.05). These small p-values suggest that these variables are significant predictors of our response variable. The only value that does not show a significant p-value is chemIndex, which we chose to keep because the interaction between chemIndex and log(cycle) is significant. This model has multiple interactions: porosity and log(cycle), frequency and log(cycle), and chemical index and log(cycle). These interactions show that if all other variables are held constant, there is a combined effect of these interactions on the response variable. 

```{r, echo = FALSE}
Anova(dampingModel)
```

We used a residuals vs fitted plot and QQ plot to assess the conditions of our model (see @fig-example5 and @fig-example6, in the Appendix). We found that this damping model meets all of these conditions which therefore we can analyze our model's results. 
...
**Question 1:** We see that frequency, amplitude, chemical index, and cycle all have significant impacts on both stiffness and damping. Porosity also has a significant impact on damping, but not on stiffness.


**Question 2:** For stiffness, we see that the individual predictors on their own will increase the initial stiffness, but as time goes on, foam using chemical index 121 will have the greatest decrease in stiffness. The individual predictors will impact damping in a similar way but foam with a chemical index of 79 and a low porosity will see the greatest decay in damping. Overall, there appear to be significant impacts on the stiffness and damping values depending on chemical index and porosity.
...
A similar concern comes from foam types and testing conditions that were used less often, such as high frequency. The reliability of our models is already limited, and for these cases they are even less reliable. Additionally, if one variety of foam was never tested at high frequency, then the model's claims about that foam at high frequency are based on nothing, and should not be trusted at all. So not all information this model gives on what effects stiffness and damping in foam is equally trustworthy. 
