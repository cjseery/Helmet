...
All samples have a stiffness fraction of 1 at cycle zero. A mixed model can create individual intercepts for each sample, so for this model, we used samples starting at the first cycle measurement after cycle zero. Thus, $\alpha$ terms in the model  explain the impact of the covariates right after compressions begin, and the $\beta$ terms explain the impact of the covariates as time goes on.

From @Table-1, we see a high significance in our coefficients. Some variables are not significant on their own, their interactions with log(cycle) are. For example, chemical index does not appear to have a significant relationship with the response on it's own, but one level of chemical index has a significant relationship when interacted with log(cycle). The interaction terms explain how log(cycle) effects stiffness fraction differently under different conditions. From this summary, amplitude appears to have the largest positive impact on initial stiffness fraction, and the largest negative impact as time progresses.
...
Although @Table-1 indicates there is significance to the different levels of chemical index, the output below in @Table-2 confirms that there is a significant difference between at least one of the levels of chemical index. To further assess the significance of the model, we used diagnostic plots to find that this model meets the linearity, normality, and equal variance conditions. These plots are in the appendix.
