...
*Caption: Table 1. Explanatory Variables*
...
*Caption: Table 2. Response Variables*
...
There were a few concerning observations with negative values of stiffness, likely due to a fault in the measuring machine. At Foster’s instruction, we converted these negative stiffness values to positive. The transformed response dampingFrac also had five concerning observations, that were not just negative, but drastic outliers. These were changed to NA per Dr. Philtron's recommendation.
...
For future studies and more reliable models, we recommend testing more foam samples to increase the number of observations that can be used. The same number of each type of foam should be used, and treatments such as amplitude and initial stress should be assigned evenly. This ensures each treatment level is fully tested on each type of foam. We would also recommend regulating the temperature of the testing environment to help provide the best testing conditions. 
