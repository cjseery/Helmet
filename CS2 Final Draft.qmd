...
The histogram in @fig-1 shows the distribution of compression frequencies among the foam samples, revealing that certain frequencies were tested much less often than others. These non-uniformities in sample distribution might impact the reliability of conclusions drawn from the analysis.
...
|:---------------------------------|:------------------------------------|
           chemIndex               |             chemIndex
             freq                  |               freq
          log(cycle)               |            log(cycle)
              amp                  |         chemIndex:log(cycle)
       chemIndex:log(cycle)        |          freq:log(cycle)
        freq:log(cycle)            |           
        amp:log(cycle)             |         
...
\begin{center}
\text{Table 7: Coefficients for Damaping Fraction Model}
\end{center}

The summary of this model shows a high significance in almost all of our coefficients through small p-values (less than 0.05). These small p-values suggest that these variables are significant predictors of our response variable. 

Like in the stiffnessFrac model, the interaction between log(cycle) and frequency has a negative coefficient, so damping decreases more at higher compression frequencies. Chemical index levels 2 and 3 (aka 100 and 121) interacted with log(cycle) also have negative coefficients, indicating that these foams lose damping faster than foam with index 79. But like in the stiffnessFrac model, these two chemical indices have higher damping right after compressions start, and foams with high frequency do as well. 

Like the stiffness model, not all chemical indices look significant in Table 7, but we can double-check our predictors in the Anova in Table 8 and see that all our predictors are significant. 
...
\begin{center}
\text{Table 8: Anova Analysis of Damping Fraction Model}
\end{center}
...
**Question 1:** We see that frequency, chemical index, and cycle all have significant impacts on both stiffness and damping. Amplitude also has an impact on stiffness.

**Question 2:** For stiffness, we see that high frequency, high amplitude, and chemical indices 100 and 121 will increase the stiffness right after compressions start, but as time goes on, these will cause the more decrease in stiffness. Foam using chemical index 121 will have the greatest decrease in stiffness, and foam using chemical index 79 will have its stiffness decrease the least. For damping, high frequency and chemical indices 100 and 121 similarly lead to higher damping right after compressions start, and lead to the fastest decay in damping. The type of foam with the greatest decay in damping is chemical index 121, and foam using chemical index 79 will have its damping decrease the least.
...
A similar concern comes from testing conditions that were used less often, such as high frequency. The reliability of our models is already limited, and for these cases they are even less reliable. Additionally, if one variety of foam was never tested at high amplitude, then the model's claims about that foam at high frequency are based on nothing, and should not be trusted at all. So not all information this model gives on what effects stiffness and damping in foam is equally trustworthy. 
